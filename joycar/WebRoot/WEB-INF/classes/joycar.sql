...
commit;
--分页查询所有管理员用户
--（1）查询所有管理员，按id降序排列(关联配置表带出中文状态名)
select 
       a.admin_Id,
       a.admin_account,
       a.admin_password,
...
select 
       admin_Id,
		   ...
		   modified_time
		   from v$admin
		   where admin_account = 'xxxx' and account_status = '0';
--(2)带出rn
select
       admin_Id,
       admin_account,
          ...
          select
                 ...
                 from(
                    select 
                           ...
                           modified_time
                           from v$admin
                           where admin_account like '%%' and account_status = '0'
                 )
       )where rn >1 and rn <= 3
...
delete from t_user where user_account = 'hexu';
commit;
--修改用户user
update t_user set
       user_account ，
...
--从视图查询全部根据账号（多条件查询user_account，phone_number，account_status）
--（1）带着条件查询
select 
       user_Id,
       user_account,
       ...
       phone_status_name
       from v$user
       where user_account like '%hex%' 
       and phone_number = '15755843966'
...
commit;
		...
		update t_phone_user set
		       phone_fee = 600,
		       modified_time =sysdate
		       where phone_number = '15755843966'
--根据id修改手机用户
update t_phone_user set
       phone_status = 600,
       modified_time =sysdate
       where phone_user_id = 1;
commit;

delete from t_phone_user;
commit;

select * from t_phone_user
--查询所有手机用户
select 
       b.phone_user_id,
       b.phone_number,
...
select
   ...
   from v$phoneuser
   where phone_number = '15755843966' 
         and phone_user_name like '%旭%'
         and phone_user_sex = '0'
         and phone_status = '0'
--(2)带出rn
select
   phone_user_id,
...
select province_id,province_name,create_time,modified_time from t_province;
...
insert into t_car(car_id,user_account,province_id,car_number,car_body_number,car_type,check_time,next_check_time,check_status,create_time,modified_time)
       values(seq_car.nextval,'hexu',1,'8888888','123456','1',sysdate ,add_months(sysdate, 12),'0',sysdate,sysdate );
insert into t_car(car_id,user_account,province_id,car_number,car_body_number,car_type,check_time,next_check_time,check_status,create_time,modified_time)
       values(seq_car.nextval,'hexu',1,'9999999','222222','0',sysdate ,add_months(sysdate, 12),'1',sysdate,sysdate );
commit;
insert into t_car          
            (car_id,           
             user_account,           
...
hexu(String), 皖NB88888888(String), hexu122121(String), 1(String), 1(String)
--新增车辆
insert into t_car
       (car_id,
        user_account,
...
--多条件分页查询（user_account:完全匹配，check_status）
--(1)带着条件查询
select        
       car_id,
       user_account,
       ...
       modified_time
       from v$car
       where user_account = 'hexu' and check_status = '0'
--(2)带出rn
select        
       car_id,
       user_account,
...
--多条件分页
--（1）查询全部从视图中(userAccount，recordNumber，carNumber，recordStatus)
select
       record_id,
       record_number,
       user_account,
       ...
       where 
            user_account = 'hexu'
            and record_number ='1B699D440115' 
            and car_number = '8888888' 
            and record_status ='0'
select * from v$record
--(2)带出rn
select
       record_id,
       record_number,
       ...
       from(
          select
                 ...
                 where 
                      user_account = 'hexu'
                      and record_number ='8FE3EAD2FD71' 
                      and car_number = '8888888' 
                      and record_status ='0'
       );
--(3)结合分页
...
select
       ...
       modified_time
       from v$rechargecard
       where card_number='A31B7471DD4E49BA9D5769F4651C1F80' and card_status='0'
--(2)带出rn
select
       card_id,
       card_number,
...
select
       ...
       modified_time
       from v$rechargecard
       where card_number='A31B7471DD4E49BA9D5769F4651C1F80' 
       and card_password ='3EDC8623FD974'
--批量添加密保卡
